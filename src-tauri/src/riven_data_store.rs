...
fn write_rivens_store(auction: Auction) -> Result<(), AppError> {
    let path: PathBuf = env::var("PWD")
        .map_err(|e| {
            AppError::new(
                e.to_string().into(),
                "write_rivens_store: env::var: ".into(),
            )
        })?
        .into();
    ...
    if path.exists() {
        fs::remove_dir_all(path.clone()).map_err(|e| {
            AppError::new(
                e.to_string().into(),
                "write_rivens_store: remove_dir_all: ".into(),
            )
        })?;
    }
    fs::create_dir(path.clone()).map_err(|e| {
        AppError::new(
            e.to_string().into(),
            "write_rivens_store: create_dir: ".into(),
        )
    })?;
    ...
    let mut file = File::create(path)
        .map_err(|e| AppError::new(e.to_string().into(), "write_rivens_store: create: ".into()))?;
    let json = serde_json::to_string_pretty::<Auction>(&auction).map_err(|e| {
        AppError::new(
            e.to_string().into(),
            "write_rivens_store: to_string_pretty: ".into(),
        )
    })?;
    file.write_all(json.as_bytes()).map_err(|e| {
        AppError::new(
            e.to_string().into(),
            "write_rivens_store: write_all: ".into(),
        )
    })?;
    Ok(())
}
